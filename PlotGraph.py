...
class Bar_Graph_Class():    
    def CreateBarGraphPlot(lca_Data, graphWidget):
        if not lca_Data:
            return graphWidget
        else:
            ...
            colors = [tuple(255*x for x in cmap(i/10))[:-1] for i in range(len(labels_Dict[keys.FABMETHOD]))]
            ...
            for factor in factors:                          
                ...
                for phase in phases:
                    ...
                    barPlot.setLabel('top', phase) # Set plot title with phase name
                    ...
                    for fabmethod, color in zip(data.keys(), colors): 
                        values = data.get(fabmethod)
                        bg = pqg.BarGraphItem(x=x, height=values[i], width=barWidth, brush=pqg.mkBrush(color), name=fabmethod)
                        barPlot.addItem(bg)
                        x += barWidth
                    i += 1
                graphWidget.nextRow()
